...
def main_worker(gpu, ngpus_per_node, argss):
    ...
    from model.models import PSPNet
    ...
    modules_new = [model.enrich_module, model.cls, model.aux]
    ...
    if args.evaluate:
        ### [warning] the transformations for validation cannot be altered !
        val_transform = transform.Compose([
            transform.AdaptImage(ignore_label=args.ignore_label),
            transform.ToTensor(),
            transform.Normalize(mean=mean, std=std)])
...
def validate(val_loader, model, criterion):
    ...
    allAcc = sum(intersection_meter.sum) / (sum(target_meter.sum) + 1e-10)
    if main_process():
        logger.info('Val result: mIou-speed/mIoU/mAcc/allAcc {:.4f}/{:.4f}/{:.4f}/{:.4f}.'.format(mIoU/(batch_time.avg), 
                                                                                                  mIoU, mAcc, allAcc))
